...
All four draft-stage and public HTTPS downloads matched the local originals and GitHub SHA-256 digests. After verification and backup, all 17 old v2.0.0 release assets (including ARM/32-bit/Windows outputs) were retired; the release page, unchanged tag, and automatic source archives remain, with replacement links. Temporarily paused workflow states were restored without unwanted rebuilds or image publication. The initial 2026-09-10 package release made no deployment changes and published no new GHCR images; the later Docker publication is recorded below. Package validation covered runtime version and package/static/hash checks, not installation smoke tests.

### Docker follow-up (2026-09-11 UTC)

The classic image `ghcr.io/hrezenmsft/rustdeskadmin-server:v2.0.1` is now published for **Linux amd64 only**. It reuses all three existing ZIP executables byte-for-byte, with **no Rust rebuild**. Packaging source/OCI revision is `8cbc59c8950e24645cdbd81cc5499f2105907330`, distinct from the unchanged binary build and Git release tag commits above. The image digest is `sha256:5020adfdfb60de969a2e2a62712f72ddf179906cde90f9a71816cdb9db06c5c1`; `v2.0.1`, `v2.0.1-amd64`, `latest`, `latest-amd64`, `v2`, and `v2-amd64` resolve to that single manifest. Pull/version checks used isolated, network-disabled, read-only containers; `hbbs` and `hbbr` report `2.0.1`.

Classic packaging retains `FROM scratch`, binaries in `/usr/bin`, and `HOME`/`WORKDIR /root`. It adds `RELEASE-NOTICE.txt` and `LICENSE` under `/usr/share/doc/rustdeskadmin-server/`, preserving upstream attribution. OCI labels identify **RustDeskAdmin Server - RustDesk Fork**, **Henrique Rezende**, `v2.0.1`, `AGPL-3.0-only`, and `https://github.com/hrezenmsft/rustdeskadmin-server`. `docker-compose.example.yml` pins both services to `:v2.0.1` and notes amd64 only; topology and API behavior remain unchanged.

Future **classic-only** CI now uses an amd64-only `docker-classic` matrix, stages license/notices, and passes `VERSION`/`VCS_REF` build arguments. `docker-manifest-classic` aliases only amd64 and correctly uses `needs: docker-classic` instead of `needs: docker`. S6, binary, and DEB release matrices are unchanged; do not infer a broader release-scope change or trigger rebuilds for this packaging follow-up.

Following a full digest-verified OCI backup of all 16 original classic package versions, 8 historical ARM images and 4 obsolete multi-platform indexes were retired. Four original amd64 images plus the new 2.0.1 image remain. Historical `v2.0.0`, `v1.1.4`, `v1`, and `v1.1.3` tags retain their original amd64 binaries; `v1.1.2-amd64` is preserved. Old ARM references and multi-platform index digests no longer work; restoring old manifest digests requires the backup. Git release tags and the **`rustdeskadmin-server-s6` package are unchanged**. Exact evidence/provenance is summarized in the [handoff](ADMIN_PRESENCE_AI_HANDOFF.md#docker-follow-up-2026-09-11-utc).

The running local **`rustdeskadmin-server:2.0.0`** deployment was not restarted, recreated, or upgraded, and its configuration was not changed. It remains excluded from these generic deployment examples.
